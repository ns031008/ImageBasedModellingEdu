                           ...
                           double z = 0.99)
{
     ...
     * Coding here**/
    int M = 0;
    M = static_cast<int>(std::log(1.0 - z) * 1.0 / std::log(1.0 - std::pow(p, K)));
    return M;
...
std::vector<int> find_inliers(sfm::Correspondences2D2D const &matches, FundamentalMatrix const &F, const double &thresh)
{
    ...
    /** TODO HERE Coding here **/

    size_t i = 0;
    for (auto match : matches)
    {
        math::Matrix<double,3,1> pt1, pt2;
        pt1(0,0) = match.p1[0];
        pt1(1,0) = match.p1[1];
        pt1(2,0) = 1.0;
        pt2(0,0) = match.p2[0];
        pt2(1,0) = match.p2[1];
        pt2(2,0) = 1.0;
        double numerator = std::pow((pt2.transposed() * F * pt1)[0], 2);
        double denominator = std::pow((F * pt1)[0], 2) + std::pow((F * pt1)[1], 2) + std::pow((pt2.transposed() * F)[0], 2) + std::pow((pt2.transposed() * F)[1], 2);
        double SampsonDistance = numerator*1.0/denominator;
        if (SampsonDistance < squared_thresh){
...
int main(int argc, char *argv[])
{
    ...
    std::ifstream in("/home/chen/桌面/三维重建/ImageBasedModellingEdu/examples/task1/correspondences.txt");
    assert(in.is_open());
