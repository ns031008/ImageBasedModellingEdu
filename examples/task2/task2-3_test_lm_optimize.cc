 ...
 *
 *   B = Jcc, E = Jcp, C = Jpp
 *  其中 Jcc = Jc^T* Jc, Jcp = Jc^T*Jp, Jpc = Jp^TJc, Jpp = Jp^T*Jp
 *      v = Jc^T(f-x), w = Jp^T(f-x), deta_x = [delta_c; delta_p]
 */
...
int main(int argc, char* argv[])
{
    ...
    /* 加载数据 */
    load_data("/home/demo/桌面/ImageBasedModellingEdu/examples/task2/test_ba.txt", cameras, points, observations);
